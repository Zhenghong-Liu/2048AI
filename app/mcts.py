...
class MCTS:
    ...
    def reset(self):
        ...
        self.gameEnv = Game2048()
        self.gameEnv.grid_n = 4
        self.ucb_debug = False
        self.predict_debug = False
...
def select_one_dir_with_model(model,gameEnv,debug):

    observation = copy.deepcopy(gameEnv.matrix)
    # if debug:gameEnv.display()
    #======================================================================================
    if False == (str(observation) in model.node_dict and model.node_dict[str(observation)].n > 20):
